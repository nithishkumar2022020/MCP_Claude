...
# Initialize FastMCP
mcp = FastMCP("WebVulnScanner")

@mcp.tool()
def scan_url(url: str) -> Dict:
    """
    Scan a URL for security vulnerabilities using FastMCP.
    
    Args:
        url: The target URL to scan
    
    Returns:
        A dictionary containing scan results in JSON format
    """
    try:
        # Initialize scanner
...
class WebSecurityScanner:
    def __init__(self, target_url: str, max_depth: int = 3):
        ...
        self.lock = Lock()  # Thread-safe access to shared resources
    ...
    def crawl(self, url: str, depth: int = 0) -> None:
        ...
        try:
            ...
            response = self.session.get(normalized_url, verify=False)
            soup = BeautifulSoup(response.text, 'html.parser')

            # Find all links in the page
            links = soup.find_all('a', href=True)
            for link in links:
                next_url = urllib.parse.urljoin(normalized_url, link['href'])
                if next_url.startswith(self.target_url):
                    self.crawl(next_url, depth + 1)

        except Exception as e:
            self.report_error(f"Error crawling {url}: {str(e)}")
    ...
    def check_sql_injection(self, url: str) -> None:
        ...
        for payload in sql_payloads:
            try:
                ...
                self.report_error(f"Error testing SQL injection on {url}: {str(e)}")
    ...
    def check_xss(self, url: str) -> None:
        ...
        for payload in xss_payloads:
            try:
                ...
                self.report_error(f"Error testing XSS on {url}: {str(e)}")
    ...
    def check_sensitive_info(self, url: str) -> None:
        ...
        try:
            ...
            self.report_error(f"Error checking sensitive information on {url}: {str(e)}")
    ...
    def scan(self) -> List[Dict]:
        ...
        # Use multithreading to perform checks
    ...
    def report_vulnerability(self, vulnerability: Dict) -> None:
        """Record found vulnerabilities."""
        with self.lock:
            self.vulnerabilities.append(vulnerability)

    def report_error(self, message: str) -> None:
        """Record errors encountered during the scan."""
        with self.lock:
            self.errors.append({'type': 'Error', 'message': message})
